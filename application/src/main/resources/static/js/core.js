(function($, window, document) {
	...
	function getJsonFromPaintedItems() {
		var jsonObject = {};
		...
		$.each(items, function(index, item) {
			var e = {};
			e.x = parseInt($(item).attr('x'));
			e.y = parseInt($(item).attr('y'));
			jsonObject.items.push(e);
		});
	...
	function sendAjaxRequest() {
		var paintedItems = getJsonFromPaintedItems();
		$.ajax({
	...
	function paintStructure(structure) {
		$.each(structure.items, function(index, item) {
			paint(item.x, item.y);
		});
	}
